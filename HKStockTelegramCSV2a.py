...
# --- SIDEBAR: Telegram credential input (only shown when not already configured) ---
...
def send_telegram_msg(winners_df, losers_df):
    ...
    payload = {
        ...
        "text": header + win_text + lose_text,
        "parse_mode": "Markdown"
    }
    try:
...
if not summary_df.empty:
    ...
    # --- Sidebar: Manual Telegram send button ---
    ...
    # --- TELEGRAM AUTO-SEND (only if credentials are available) ---
    ...
    with tab2:
        if not tech_df.empty:
            for t_type, icon in [('Bullish', '📈'), ('Bearish', '📉')]:
                ...
                if not sub.empty:
                    st.dataframe(
                        sub.style.map(style_stoch, subset=['Stoch']),
